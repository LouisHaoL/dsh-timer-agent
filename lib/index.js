...
function normalizeSchedule(schedule) {
	...
	if (!isIntervalRule({ intervalMinutes }) && (rule.cron.trim() === "" || !isValidCron(rule.cron))) return void 0;
	return {
		enabled: rule.enabled === true && (rule.cron.trim() !== "" || intervalMinutes !== void 0),
...
function summarize(job) {
	...
	if (job.schedule?.enabled === true) {
		const schedule = isIntervalRule(job.schedule) ? { interval_minutes: job.schedule.intervalMinutes } : { cron: job.schedule.cron };
	...
	return tools.register(defineTool({
		...
		async execute(args) {
			...
			if (action === "create") {
				...
				if (cron === "" && intervalMinutes === void 0) return {
					kind: "create",
					error: "schedule (cron) or interval_minutes is required for create"
				};
			...
			if (action === "pause" || action === "resume") {
				...
				const updated = await deps.store.mutate((jobs) => {
					...
					const nextRunAt = enabled ? isIntervalRule(job.schedule) ? now() + job.schedule.intervalMinutes * 6e4 : nextRunAtMs(job.schedule.cron, now()) : void 0;
			...
			if (action === "archive" || action === "restart") {
				const updated = await deps.store.mutate((jobs) => {
					...
					if (next.schedule?.enabled === true && isSchedulable(next.schedule)) {
						const armed = isIntervalRule(next.schedule) ? now() + next.schedule.intervalMinutes * 6e4 : nextRunAtMs(next.schedule.cron, now());
						next = withSchedule(next, {
							enabled: true,
							nextRunAt: armed
						}, now());
				...
				if (method === "PATCH") {
					...
					const updated = await deps.store.mutate((jobs) => {
						...
						if (body.scheduleEnabled === true) {
							...
							const armed = isIntervalRule(schedule) ? deps.now() + schedule.intervalMinutes * 6e4 : nextRunAtMs(schedule.cron, deps.now());
						...
						if (body.restart === true && next.status === "archived") {
							...
							if (next.schedule?.enabled === true && isSchedulable(next.schedule)) {
								const armed = isIntervalRule(next.schedule) ? deps.now() + next.schedule.intervalMinutes * 6e4 : nextRunAtMs(next.schedule.cron, deps.now());
								if (armed === void 0) return void 0;
								next = withSchedule(next, {
									enabled: true,
									nextRunAt: armed
